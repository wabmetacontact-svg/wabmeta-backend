...
router.post("/verify", async (req, res, next) => {
    try {
        ...
        const signatureOk = sigBuf.length === expectedBuf.length && crypto_1.default.timingSafeEqual(sigBuf, expectedBuf);
        if (!signatureOk) {
        ...
        const orderPlanKey = String(order.notes?.planKey || "").toLowerCase().trim();
        const orderOrgId = String(order.notes?.organizationId || "");
        if (!orderPlanKey) {
            throw new errorHandler_1.AppError("Order is missing plan information. Please start checkout again.", 400);
        }
        // The order must belong to the organization that is claiming it.
        if (orderOrgId && orderOrgId !== organizationId) {
            throw new errorHandler_1.AppError("This payment belongs to a different organization", 403);
        }
        if (order.status !== "paid") {
            throw new errorHandler_1.AppError(`Order is not paid (status: ${order.status})`, 400);
        }
        const normalizedKey = orderPlanKey;
        const selected = exports.PLAN_KEY_MAP[normalizedKey];
        if (!selected) {
            throw new errorHandler_1.AppError(`Invalid plan on order: ${orderPlanKey}`, 400);
        }
        // Belt and braces: the amount Razorpay captured must match the plan's price.
        if (Number(order.amount) !== selected.amount) {
            throw new errorHandler_1.AppError(`Paid amount does not match the ${selected.label} plan price`, 400);
        }
        ...
        if (bodyPlanKey && bodyPlanKey !== normalizedKey) {
            console.warn(`⚠️ planKey mismatch — body said "${bodyPlanKey}", order says "${normalizedKey}". Using the order.`);
        }
